...
function applyTexts(){
  ...
  const confs=Object.keys(directory.conferences||{}).sort();$("conference").innerHTML=`<option value="">All Conferences</option>`+confs.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join("");$("conference").value=c;
}
